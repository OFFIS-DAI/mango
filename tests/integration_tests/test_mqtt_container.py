...
from mango.messages.message import ACLMessage as ACLMessage, Performatives
...
class SimpleMQTTAgent(Agent):
        ...
        if self.connection_type == "mqtt":
            ...
            if self.acl:
                await self._container.send_message(
                    ...
                    receiver_addr=self.other_addr,
                    receiver_id=self.other_aid,
                    acl_metadata=message_meta,
                    create_acl=True,
                )
            else:
                msg = ACLMessage()
                ...
                for key, value in FIRST_GREETING_META.items():
    ...
    async def react_to_greeting(self, content_in, meta_in):
        ...
        # greet back
        print("meta_in is: ", meta_in)
        print("content_in is: ", content_in)
        ...
        if self.connection_type == "mqtt":
            await self._container.send_message(
                receiver_addr=sender_addr, content=message
            )
        elif self.connection_type == "tcp":
            # TODO
            pass
        ...
        if self.connection_type == "mqtt":
            if self.acl:
                await self._container.send_message(
                    ...
                    receiver_id=meta_in["sender_id"],
                    receiver_addr=meta_in["sender_addr"],
                    create_acl=True,
                )
            else:
                msg = ACLMessage()
                msg.content = message_out_content
                for key, value in GREETING_ANSWER_META.items():
                    msg.__setattr__(key, value)
...
async def simple_mqtt_agents_setup(
    no_container,
    broker,
    inbox_1="inbox_1",
    inbox_2="inbox_2",
    ...
    if no_container > 1:
        container2 = await Container.factory(
            connection_type="mqtt",
            addr=inbox_2,
            codec=codec,
    ...
    agent_b = SimpleMQTTAgent(
        container2,
        other_addr=agent_a.my_topic,
        other_aid=agent_a.aid,
        codec=codec,
        acl=acl,
    )
    if add_topic_2:
    ...
    try:
        ...
        await asyncio.wait_for(container1._no_agents_running, 3)
        if no_container == 2:
            await asyncio.wait_for(container2._no_agents_running, 3)
...
async def test_two_container_add_topics_json():
    await simple_mqtt_agents_setup(
        2,
        ("localhost", 1883, 60),
...
async def test_one_container_mqtt_proto():
    await simple_mqtt_agents_setup(1, ("localhost", 1883, 60), codec=PROTOBUF())
...
async def test_two_container_mqtt_proto():
    await simple_mqtt_agents_setup(2, ("localhost", 1883, 60), codec=PROTOBUF())
